...
## 2026-05-28 - [DOM Traversal vs QuerySelector]
**Learning:** `querySelectorAll` is O(N) and forces a full subtree scan. For linear lists, navigating via `nextElementSibling` (O(1)) removes latency that scales with list size.
**Action:** Prefer direct DOM relationship traversal (`closest`, `nextElementSibling`) over selector queries for keyboard navigation in potentially long lists.
