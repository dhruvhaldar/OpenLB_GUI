...
const HighlightMatch = ({ text, regex }: { text: string; regex?: RegExp | null }) => {
...
const SidebarListItems = memo(({ cases, selectedId, onSelect, searchTerm }: SidebarListItemsProps) => {
  // Bolt Optimization: Memoize the regex creation to avoid creating it for every item
  // This reduces regex compilations from O(N) to O(1) per render frame during filtering.
