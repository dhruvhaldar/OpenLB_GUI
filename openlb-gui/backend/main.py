...
# Optimization: Pre-defined ignore patterns for case duplication
# Using a tuple for extensions allows for fast C-level endswith() check
IGNORED_EXTENSIONS = (
    ".o", ".obj", ".a", ".so", ".dll", ".exe",  # Build artifacts
    ".vtk", ".vti", ".vtu", ".vtp", ".pvti", ".pvtu", # Simulation outputs
    ".log", ".out", ".err"  # Logs
)
# Optimization: Include lowercase .ds_store for Windows case-insensitive match logic
IGNORED_DIRS = {"tmp", "__pycache__", ".git", ".DS_Store", ".ds_store"}
...
def fast_ignore_patterns(path, names):
    ...
    This implementation is O(N) using set lookups and optimized tuple endswith().
    """
    ignored = set()
    for name in names:
        # Check against directories (exact match)
        # Handle case-sensitivity based on OS to match fnmatch behavior (Windows is insensitive)
        check_name = name.lower() if os.name == 'nt' else name

        if check_name in IGNORED_DIRS:
            ignored.add(name)
        # Optimization: endswith with tuple is significantly faster than fnmatch
        elif check_name.endswith(IGNORED_EXTENSIONS):
            ignored.add(name)
...
def duplicate_case(req: DuplicateRequest, request: Request):
    ...
    try:
        ...
        # This significantly speeds up duplication for cases with many files.
